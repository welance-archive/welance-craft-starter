...
if [ ! -d "$CRAFT_PLUGINS_PATH/sproutimport" ]; then
 ...
 wget https://sprout.barrelstrengthdesign.com/craft-plugins/import/download?v= -O $SPROUT_FILE
 unzip -q $SPROUT_FILE
 rm $SPROUT_FILE
fi
...
if [ -f $ENV_LIVE ]; then
 ...
 OUT=1
 while [ $OUT -eq 1 ]; do
  php -f /data/poll-db.php $CRAFT_CONFIG_PATH/db.php $CRAFT_ENVIRONMENT
  OUT=$?
  echo "connection unavailable waiting 3 seconds before retry"
  sleep 3
 done
 /data/craft/plugins/vendor/bin/schematic import --file=/data/craft/config/schema.yml
fi
